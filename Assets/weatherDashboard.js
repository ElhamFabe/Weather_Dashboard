$(document).ready(function () {
    ...
    // create weather description results
    ...
    $('#weathersubmit').on("click", function (event) {
        ...
        // apppend search results to weather description container
...
function getWeather (city){
    ...
    $.ajax({
        ...
        $.ajax({
            ...
            for (var i = 1; i < 6; i++) {
                var htmlStr = ` <h1 id="description">${data.daily[i].weather[0].description}</h1>
                <p id="tempMin">${data.daily[i].temp.min}</p>
                <p id="tempMax"></p>
                <p id="humidity"></p>
                <p id="uvIndex"></p>
                <p id="windSpeed"></p>`
